...
interface TourGuideProps {
  ...
  onAnnotationHighlight: (annotationId: string, preventZoom?: boolean) => void;
}
...
export default function TourGuide({
  ...
  const goToStep = (index: number) => {
    ...
    if (step.target_annotation_id || step.linked_annotation_id) {
      onAnnotationHighlight(step.target_annotation_id || step.linked_annotation_id, true);
    }
  };
      ...
      {!isActive ? (
        ...
        <div className="tour-player">
          ...
          <div className="tour-narration">
            <div className="narration-step" style={{ marginBottom: '8px', fontSize: '13px', color: '#666' }}>
              第 {currentStepIndex + 1} / {steps.length} 步
            </div>
            {currentStepAny?.title && (
              <h3 style={{ margin: '0 0 10px 0', fontSize: '18px', color: '#8b4513' }}>
                {currentStepAny.title}
              </h3>
            )}
            <div className="narration-text" style={{ fontSize: '16px', lineHeight: '1.6', color: '#2c2c2c', fontWeight: '500' }}>
